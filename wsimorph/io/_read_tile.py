...
def read_tile(wsi: WSI, level: int, y_start: int, x_start: int, y_len: int, x_len: int, padding: int) -> Tile:
    """
    ...
    if y_start + y_len > level_dimensions[0] or x_start + x_len > level_dimensions[1]:
        raise ValueError("y_start + y_len exceeds the level dimensions or x_start + x_len exceeds the level dimensions.")
    ...
    with OpenSlide(wsi.path) as slide:
        image = np.array(slide.read_region((x_start_tile, y_start_tile), level, (x_len_tile, y_len_tile)).convert("RGB"))
    ...
    return Tile(tile_image, y_start_tile, x_start_tile, level, ylo_pad, yhi_pad, xlo_pad, xhi_pad, wsi)
